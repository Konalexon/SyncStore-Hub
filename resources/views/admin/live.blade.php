...
@section('content')
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-8">
                ...
                <div class="card border-0 shadow-sm rounded-4 mt-4">
                    <div class="card-header bg-white p-3 border-bottom-0">
                        <h5 class="mb-0 fw-bold"><i class="bi bi-bar-chart-line me-2"></i>Analytics Dashboard</h5>
                    </div>
                    <div class="card-body p-4">
                        <div class="row g-4">
                            <!-- Viewer Graph -->
                            <div class="col-md-8">
                                <h6 class="fw-bold text-muted small mb-3">LIVE VIEWERS</h6>
                                <canvas id="viewerChart" style="height: 200px;"></canvas>
                            </div>
                            <!-- Conversion Funnel -->
                            <div class="col-md-4">
                                <h6 class="fw-bold text-muted small mb-3">CONVERSION</h6>
                                <canvas id="conversionChart" style="height: 200px;"></canvas>
                            </div>
                        </div>
                        <div class="row g-4 mt-2">
                            <!-- Chat Activity Heatmap -->
                            <div class="col-12">
                                <h6 class="fw-bold text-muted small mb-3">CHAT ACTIVITY (Messages/Min)</h6>
                                <canvas id="chatHeatmap" style="height: 100px;"></canvas>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script type="module">
        import { StreamBroadcaster } from '/js/stream-simulation.js';
    ...
    <script type="module">
        ...
        let viewerChart, conversionChart, chatHeatmap;

        function initCharts() {
            // Viewer Chart (Line)
            const ctxViewer = document.getElementById('viewerChart').getContext('2d');
            viewerChart = new Chart(ctxViewer, {
                type: 'line',
                data: {
                    labels: [],
                    datasets: [{
            ...
            // Chat Heatmap (Bar)
            const ctxChat = document.getElementById('chatHeatmap').getContext('2d');
            chatHeatmap = new Chart(ctxChat, {
                type: 'bar',
                data: {
                    labels: [],
                    datasets: [{
        ...
        async function updateAnalytics() {
            try {
                // Assuming stream ID 1 for now, or fetch dynamically
                const response = await axios.get('/analytics/stats/1'); 
                const data = response.data;

                // Update Viewer Chart
                viewerChart.data.labels = data.viewer_history.map(h => h.time);
                viewerChart.data.datasets[0].data = data.viewer_history.map(h => h.count);
                viewerChart.update();

                // Update Conversion Chart
                conversionChart.data.datasets[0].data = [data.conversion.clicks, data.conversion.purchases];
                conversionChart.update();

                // Update Chat Heatmap
                chatHeatmap.data.labels = data.chat_activity.map(c => c.minute);
                chatHeatmap.data.datasets[0].data = data.chat_activity.map(c => c.count);
                chatHeatmap.update();
        ...
        // Initialize
        initCharts();
        setInterval(updateAnalytics, 5000); // Poll every 5s
    </script>
@endsection